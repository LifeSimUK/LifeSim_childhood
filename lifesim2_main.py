...
# Read saved copies of all packages required.
import os
os.chdir(lfsm)
...
def cstsm(desc, r, s, cs, ctp="CT"):
     # desc - input data (descriptive statistics from sample)
     # r - discount rate
     # s - risk factor start sweep
     # cs - cohort size (for cohort total calculations)
    ...
    if s == 0 or s == 1:
        n = 15 #number of years 3 to 17 incluseive = 15
        #From birth - calculating effects from age 3
        ...
        n = 14 #number of years 4 to 17 incluseive = 14
        #From age 3 calculating effects from age 4 (using age 5 outcomes)
        weights = {'3': pow((1/(1+r)), 1)+pow((1/(1+r)), 2)+pow((1/(1+r)), 3),
                   '4': pow((1/(1+r)), 4)+pow((1/(1+r)), 5)+pow((1/(1+r)), 6),
                   '5': pow((1/(1+r)), 7)+pow((1/(1+r)), 8)+pow((1/(1+r)), 9),
                   '6': pow((1/(1+r)), 10)+pow((1/(1+r)), 11)+pow((1/(1+r)), 12),
                   '7': pow((1/(1+r)), 13)+pow((1/(1+r)), 14)}
    elif s == 3:
        n = 12 #number of years 6 to 17 incluseive = 12
        #From age 5 calculating effects from age 6 (using age 7 outcomes)
        weights = {'4': pow((1/(1+r)), 1)+pow((1/(1+r)), 2)+pow((1/(1+r)), 3)+pow((1/(1+r)), 4),
                   '5': pow((1/(1+r)), 5)+pow((1/(1+r)), 6)+pow((1/(1+r)), 7),
                   '6': pow((1/(1+r)), 8)+pow((1/(1+r)), 9)+pow((1/(1+r)), 10),
                   '7': pow((1/(1+r)), 11)+pow((1/(1+r)), 12)} 
    elif s == 4:
        n = 9 #number of years 8 to 17 incluseive = 9
        #From age 7 calculating effects from age 8 (using age 11 outcomes)
        weights = {'5': pow((1/(1+r)), 1)+pow((1/(1+r)), 2)+pow((1/(1+r)), 3)+pow((1/(1+r)), 4),
                   '6': pow((1/(1+r)), 5)+pow((1/(1+r)), 6)+pow((1/(1+r)), 7),
                   '7': pow((1/(1+r)), 8)+pow((1/(1+r)), 9)}
    elif s == 5:
        n = 6 #number of years 12 to 17 incluseive = 6
        #From age 11 calculating effects from age 12 (using age 14 outcomes)
        weights = {'6': pow((1/(1+r)), 1)+pow((1/(1+r)), 2)+pow((1/(1+r)), 3)+pow((1/(1+r)), 4),
                   '7': pow((1/(1+r)), 5)+pow((1/(1+r)), 6)} 
    elif s == 6:
        n = 3 #number of years 15 to 17 incluseive = 3
        #From age 14 calculating effects from age 15 (using age 17 outcomes)
        weights = {'6': pow((1/(1+r)), 1)+pow((1/(1+r)), 2)+pow((1/(1+r)), 3)}  
...
def aswpwtot(data, varl, r, s, e):
    #data - input data
    #varl - list of variables whose total is required
    #s- risk factor start sweep
    #e- last MCS sweep included
    ##### Adding costs (Ages)
    #Wave 1 - 3 Years (9 months -> 0, 1, 2)
    #Wave 2 - 2 Years ( 3 ->  3,  4)
    #Wave 3 - 2 Years ( 5 ->  5,  6)
...
def lfstfm(desc, r, s, cs, val, ctp="CT"):
     # desc - input data (descriptive statistics from sample)
     # r - discount rate
     # s - risk factor start sweep
     # val - vlaue of 1 WELLBY
     # cs - cohort size (for cohort total calculations)
     #Choose results format to report (Default - CT)
     # A - Annual average cost per child 
     # T - Total cost per child
...
mcs = pd.read_stata(os.path.join(lfsm,'mcs1ic.dta')) #Read MCS data
...
nwvs = 7
# Choose risk factors
# rskfctrs = ["linc0", "lincim0", "lincnc0",
#             "incqnt0", "incqntim0", "incqntnc0",
#             "teenbrth0", "teenbrthim0", "teenbrthnc0",
#             "pretrmbrth0", "pretrmbrthim0", "pretrmbrthnc0",
#             "lwbrthwt0", "lwbrthwtim0", "lwbrthwtnc0"]

# rskfctrs = ["incqnt0", "incqntim0", "incqntnc0",
#             "linc0", "lincim0", "lincnc0"]

# rskfctrs = ["lincqnt0", "lincqntim0", "lincqntnc0"]

# rskfctrs = ["lwht2sd3", "lwht2sdim3", "lwht2sdnc3",
#             "lwht15sd3", "lwht15sdim3", "lwht15sdnc3",
#             "dsblty3", "dsbltyim3", "dsbltync3"]

# rskfctrs = ["lwht", "lwhtim", "lwhtnc",
#             "deldev", "deldevim", "deldevnc",
#             "disab", "disabim", "disabnc"]

# rskfctrs = ["teenbrth0", "pretrmbrth0", "lwbrthwt0", 
#             "dsblty3", "lwht3", "deldev3", "lwht15sd3"]

# rskfctrs = ["teenbrth0", "pretrmbrth0", "lwbrthwt0", 
#             "dsblty3", "lwht3", "deldev3",
#             "linc0", "incqnt0"]

rskfctrs = ["linctb0"]

# rskfctrs = ["teenbrth0", "teenbrthnm", "teenbrthnc0",
#             "pretrmbrth0", "pretrmbrthnm", "pretrmbrthnc0",
#             "lwbrthwt0", "lwbrthwtnm", "lwbrthwtnc0"]
...
### Cohort size for calculations

## 2023 Birth cohort size
# Exact
# cohsz = 
# Approximate
# cohsz = 700000

## 2000 Birth cohort size
# Exact
cohsz = 679029
# Approximate
# cohsz = 700000
...
for rskfctr in rskfctrs:
    ...
    msc = ['income123', 'incqnt123', 'aincome123', 'laincome123',
           ...
           'pretrm_1', 'pretrm_2', 'pretrm_3',
           'bthwt_1', 'bthwt_2',
           'lwht3', 'lwht15sd3', 'lwht2sd3',
           'alc3',
           'deldev3'
            ]
    if rskfctr in ("linctb0", "linctbim0", "lincnctb0"):
        msc = ["ils_dispy_uk_2327_std_ne_wt", "ils_dispy_ahc_2327_std_ne_wt", "ils_dispy_ahcit_2327_std_ne_wt",
                "ils_origy_2327_std_ne_wt", "ils_dispy_uk_2327_1_std_ne_wt", "ils_dispy_uk_2327_2_std_ne_wt",
                "ils_dispy_uk_2327_3_std_ne_wt", "ils_dispy_uk_2327_4_std_ne_wt", "ils_origy_2327_1_std_ne_wt",
                "ils_origy_2327_2_std_ne_wt", "ils_origy_2327_3_std_ne_wt", "ils_origy_2327_4_std_ne_wt",
                "ils_dispy_uk_2327_std_wt", "ils_dispy_ahc_2327_std_wt", "ils_dispy_ahcit_2327_std_wt",
                "ils_origy_2327_std_wt", "ils_dispy_uk_2327_1_std_wt", "ils_dispy_uk_2327_2_std_wt",
                "ils_dispy_uk_2327_3_std_wt", "ils_dispy_uk_2327_4_std_wt", "ils_origy_2327_1_std_wt",
                "ils_origy_2327_2_std_wt", "ils_origy_2327_3_std_wt", "ils_origy_2327_4_std_wt",
                "_merge1", "ils_dispy_uk_2327_std_ne", "ils_dispy_ahc_2327_std_ne",
                "ils_dispy_ahcit_2327_std_ne", "ils_origy_2327_std_ne", "ils_dispy_uk_2327_1_std_ne",
                "ils_dispy_uk_2327_2_std_ne", "ils_dispy_uk_2327_3_std_ne", "ils_dispy_uk_2327_4_std_ne",
                "ils_origy_2327_1_std_ne", "ils_origy_2327_2_std_ne", "ils_origy_2327_3_std_ne",
                "ils_origy_2327_4_std_ne", "ils_dispy_uk_2327_std", "ils_dispy_ahc_2327_std",
                "ils_dispy_ahcit_2327_std", "ils_origy_2327_std", "ils_dispy_uk_2327_1_std",
                "ils_dispy_uk_2327_2_std", "ils_dispy_uk_2327_3_std", "ils_dispy_uk_2327_4_std",
                "ils_origy_2327_1_std", "ils_origy_2327_2_std", "ils_origy_2327_3_std",
                "ils_origy_2327_4_std", "_merge2", "_merge3", "_merge4", "_merge5",
                "_merge6", "_merge7", "_merge8", "_merge9", "_merge10", "_merge11", "_merge12"] + msc
    ...
    sim_probs = []
    bop = boutcomes + [f"condis{i}" for i in range(swpstn + 1, nwvs + 1)]
    ...
    #### Log Income
        ...
        #                                             "mcst['laincome123'] = np.log(mcst['aincome123'])"}
        #                 ]
    #### Log Income (quintile interactions, tax benefit)
    elif rskfctr in ("lincqnt1", "lincqntim1", "lincqntnc1"):    
        interventions = [
            {'id': 'int0', 'name': 'linctbb', 'code': "mcst['recip'] = 0\n"
                                                      "mcst['aincome123'] = mcst['ils_dispy_uk_2327_std_wt']*12\n"
                                                      "mcst['laincome123'] = np.log(mcst['aincome123'])\n"
                                                      "mcst['nwmninc123'] = mcst['ils_dispy_uk_2327_std_wt']"},
            {'id': 'int1', 'name': 'linctb1', 'code': "mcst['recip'] = 0\n"
                                                      "mcst['recip'] = np.where(mcst['ils_dispy_uk_2327_std_wt'] == mcst['ils_dispy_uk_2327_1_std_wt'], mcst['recip'], 1)\n"
                                                      "mcst['aincome123'] = mcst['ils_dispy_uk_2327_1_std_wt']*12\n"
                                                      "mcst['laincome123'] = np.log(mcst['aincome123'])\n"
                                                      "mcst['nwmninc123'] = mcst['ils_dispy_uk_2327_std_wt']"},
            {'id': 'int2', 'name': 'linctb2', 'code': "mcst['recip'] = 0\n"
                                                      "mcst['recip'] = np.where(mcst['ils_dispy_uk_2327_std_wt'] == mcst['ils_dispy_uk_2327_2_std_wt'], mcst['recip'], 1)\n"
                                                      "mcst['aincome123'] = mcst['ils_dispy_uk_2327_2_std_wt']*12\n"
                                                      "mcst['laincome123'] = np.log(mcst['aincome123'])\n"
                                                      "mcst['nwmninc123'] = mcst['ils_dispy_uk_2327_std_wt']"},
            {'id': 'int3', 'name': 'linctb3', 'code': "mcst['recip'] = 0\n"
                                                      "mcst['recip'] = np.where(mcst['ils_dispy_uk_2327_std_wt'] == mcst['ils_dispy_uk_2327_3_std_wt'], mcst['recip'], 1)\n"
                                                      "mcst['aincome123'] = mcst['ils_dispy_uk_2327_3_std_wt']*12\n"
                                                      "mcst['laincome123'] = np.log(mcst['aincome123'])\n"
                                                      "mcst['nwmninc123'] = mcst['ils_dispy_uk_2327_std_wt']"}
                        ]   
    #### Log Income (tax benefit)
        ...
        interventions = [
            ...
            {'id': 'int3', 'name': 'linctb3', 'code': "mcst['recip'] = 0\n"
                                                      ...
                                                      "mcst['laincome123'] = np.log(mcst['aincome123'])\n"
                                                      "mcst['nwmninc123'] = mcst['ils_dispy_uk_2327_std_wt']"},
            {'id': 'int3', 'name': 'linctb4', 'code': "mcst['recip'] = 0\n"
                                                      "mcst['recip'] = np.where(mcst['ils_dispy_uk_2327_std_wt'] == mcst['ils_dispy_uk_2327_4_std_wt'], mcst['recip'], 1)\n"
                                                      "mcst['aincome123'] = mcst['ils_dispy_uk_2327_4_std_wt']*12\n"
                                                      "mcst['laincome123'] = np.log(mcst['aincome123'])\n"
                                                      "mcst['nwmninc123'] = mcst['ils_dispy_uk_2327_std_wt']"}
                        ]      
    #### Income
    ...
    #### Low height at age 5(2sd)
    elif rskfctr in ("lwht2sd3", "lwht2sdim3", "lwht2sdnc3"):
        interventions = [
            {'id': 'int0', 'name': 'lhtb', 'code': "mcst['recip'] = 0"},
            {'id': 'int1', 'name': 'lht1', 'code': "mcst['recip'] = np.where(mcst['lwht2sd3'] == 1, 1, 0)\n"
                                                   "mcst['lwht2sd3'] = np.where(mcst['lwht2sd3'] == 1, 0, mcst['lwht2sd3'])"}
                        ]
    #### Low height at age 5(1.5sd)
    elif rskfctr in ("lwht15sd3", "lwht15sdim3", "lwht15sdnc3"):
        interventions = [
            {'id': 'int0', 'name': 'lhtb', 'code': "mcst['recip'] = 0"},
            {'id': 'int1', 'name': 'lht1', 'code': "mcst['recip'] = np.where(mcst['lwht15sd3'] == 1, 1, 0)\n"
                                                   "mcst['lwht15sd3'] = np.where(mcst['lwht15sd3'] == 1, 0, mcst['lwht15sd3'])"}
                        ]
    #### Disability at age 5
    ...
    from Person_edt13 import Person1 # type: ignore
    ...
    for intervention in interventions:
        ...
        hst_pth = lfsm + f"/output/{rskfctr}/files/" + time.strftime("%Y%m%d-%H%M%S") + "_history_nu" + str(num_universes) + intervention['name'] + ".csv"
        ...
        for n in tqdm(range(num_universes)):
            ...
            est = simulate_person(mcst, sim_betas[n], sim_res[n], sim_probs[n], coutcomes, boutcomes, swpstn)
            est.insert(0, 'simulation', n)
            
            # Append mcsr['wt_uk2'] to the end of est
            cols = ["wt_uk2", "incqnt123_1", "incqnt123_2", "incqnt123_3", "incqnt123_4", "incqnt123_5",
                    "incqnt123", "teenbrth", "pretrm_1", "pretrm_2", "pretrm_3",
                    "bthwt_1", "bthwt_2", "lwht3", "lwht15sd3", "alc3", "deldev3",
                    "aincome123", "laincome123"]

            for col in cols:
                if col in mcst.columns:
                    est[col] = mcst[col]
                else:
                    print(f"⚠️ Warning: '{col}' not found in mcst")
                    
            if rskfctr in ("incqnt0", "incqntim0", "incqntnc0", "linctb0"):
                est['nwmninc123'] = mcst['nwmninc123']
            #For merge with LifeSim 1
            cols = ["income123", "income1", "income2",
                    "income3", "income6", "wealth5", "wealth6", "imddec6", "male",
                    "numchld1", "numchld2", "numchld3", "recip"]
            ...
            if rskfctr in ("linctb0", "linctbim0", "linctbnc0"):
                vars_list = ["ils_dispy_uk_2327_std_ne_wt", "ils_dispy_ahc_2327_std_ne_wt", "ils_dispy_ahcit_2327_std_ne_wt",
                            "ils_origy_2327_std_ne_wt", "ils_dispy_uk_2327_1_std_ne_wt", "ils_dispy_uk_2327_2_std_ne_wt",
                            "ils_dispy_uk_2327_3_std_ne_wt", "ils_dispy_uk_2327_4_std_ne_wt", "ils_origy_2327_1_std_ne_wt",
                            "ils_origy_2327_2_std_ne_wt", "ils_origy_2327_3_std_ne_wt", "ils_origy_2327_4_std_ne_wt",
                            "ils_dispy_uk_2327_std_wt", "ils_dispy_ahc_2327_std_wt", "ils_dispy_ahcit_2327_std_wt",
                            ...
                            "ils_origy_2327_1_std", "ils_origy_2327_2_std", "ils_origy_2327_3_std",
                            "ils_origy_2327_4_std", "_merge2", "_merge3", "_merge4", "_merge5",
                            "_merge6", "_merge7", "_merge8", "_merge9", "_merge10", "_merge11", "_merge12"]
        ...
        varl = ['ll_cst', 'hc_cst', 'nhs_cst', 'tot_cst']
        history = aswpwtot(history, varl, 0.035, swpstn, nwvs)
        
        varl = ['lifesat']
        history = aswpwtot(history, varl, 0.0, swpstn, nwvs)
        
        for x in range(swpstn+1, nwvs+1):
            history[f'lifesatval{x}'] = history[f'lifesat{x}'] * wbval.loc['wbnv', 'ttl']
        varl = ['lifesatval']
        history = aswpwtot(history, varl, 0.035, swpstn, nwvs)
    ...
    for rtp in ["CT", "RT"]:
        ...
        tex_path = os.path.join(lfsm, f'output/{rskfctr}/tables', f'otwdif_{rskfctr}_{rtp}.tex')  # Define your output file path here
    ...
    for rtp in ["CT", "RT"]:
        ...
        lfstfm(wb, 0.035, swpstn, cohsz, wbval.loc['wbnv', 'ttl'])
        
        logging.info('Intervention effects on wellbeing (undiscounted)')
        lfstfm(wb, 0, swpstn, cohsz, wbval.loc['wbnv', 'ttl'])
    ...
    for rtp in ["CT", "RT"]:
        ...
        tex_path = os.path.join(lfsm, f'output/{rskfctr}/tables', f'cstswdif_{rskfctr}_{rtp}.tex')  # Define your output file path here
        idf.to_latex(tex_path,
                    index=True,
                    formatters={"name": str.upper},
                    float_format="{:.3f}".format,
    ...
    for rtp in ["CT", "RT"]:
        ...
        tex_path = os.path.join(lfsm, f'output/{rskfctr}/tables', f'cstowdif_{rskfctr}_{rtp}.tex')  # Define your output file path here
        idf.to_latex(tex_path,
                    index=True,
                    formatters={"name": str.upper},
                    float_format="{:.3f}".format,
    ...
    for rtp in ["CT", "RT"]: 
        ...
        if rtp in ("CT"):
            ...
            desc = diffdescuw(varl, nint, neg = True, ppt = True, rec = True)
        del desc['mean']
        del desc['std']
        ...
        # logging.info(desc[['int1', 'int2', 'int3', 'int4']])
        # Plotting bar plots for comparison
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures' f'inc17ocdif_{rskfctr}_{rtp}.png')
        lbl = ["Poor GCSEs", "Psych. distress", "Obesity", "Regular smoker", "Poor health"]
        ...
        logging.info(desc)
        # Plotting bar plots for comparison
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'inc17ncdif_{rskfctr}_{rtp}.png')
        lbl = ["Poor GCSEs", "Psych. distress", "Obesity", "Regular smoker", "Poor health"]
        ...
        if rtp in ("CT"):
            ...
            desc = diffdescuw(varl, nint, neg = True, pct = True, rec = True)
        logging.info('Intervention effects on adverse outcomes - percent')
        logging.info(desc)
        # logging.info(desc[['int1', 'int2', 'int3', 'int4']])
        # Plotting bar plots for comparison
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'inc17pcdif_{rskfctr}_{rtp}.png')
        lbl = ["Poor GCSEs", "Psych. distress", "Obesity", "Regular smoker", "Poor health"]
        ...
        # logging.info(desc[['int1', 'int2', 'int3', 'int4']])
        # Plotting means
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'zcogdif_{rskfctr}_{rtp}.png')
        # lbl = ["2", "3", "4", "5", "6", "7"]
        # lbl = ["3", "5", "7", "11", "14", "17"]
        lbl = ["Age 3", "Age 5", "Age 7", "Age 11", "Age 14", "Age 17"][(swpstn + 1 - 2):]
        ...
        # logging.info(desc[['int1', 'int2', 'int3', 'int4']])
        # Plotting means
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'intrnldif_{rskfctr}_{rtp}.png')
        # lbl = ["2", "3", "4", "5", "6", "7"]
        # lbl = ["3", "5", "7", "11", "14", "17"]
        lbl = ["Age 3", "Age 5", "Age 7", "Age 11", "Age 14", "Age 17"][(swpstn + 1 - 2):]
        ...
        # logging.info(desc[['int1', 'int2', 'int3', 'int4']])
        # Plotting means
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'sdqextdif_{rskfctr}_{rtp}.png')
        # lbl = ["2", "3", "4", "5", "6", "7"]
        # lbl = ["3", "5", "7", "11", "14", "17"]
        lbl = ["Age 3", "Age 5", "Age 7", "Age 11", "Age 14", "Age 17"][(swpstn + 1 - 2):]
        ...
        logging.info(desc)
        # Plotting means with error bars
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'hospdif_{rskfctr}_{rtp}.png')
        # lbl = ["2", "3", "4", "5", "6", "7"]
        # lbl = ["3", "5", "7", "11", "14", "17"]
        lbl = ["Age 3", "Age 5", "Age 7", "Age 11", "Age 14", "Age 17"][(swpstn + 1 - 2):]
        ...
        logging.info(desc)
        # Plotting means with error bars
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'disdif_{rskfctr}_{rtp}.png')
        # lbl = ["2", "3", "4", "5", "6", "7"]
        # lbl = ["3", "5", "7", "11", "14", "17"]
        lbl = ["Age 3", "Age 5", "Age 7", "Age 11", "Age 14", "Age 17"][(swpstn + 1 - 2):]
        ...
        logging.info(desc)
        # Plotting means with error bars
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'condisdif_{rskfctr}_{rtp}.png')
        # lbl = ["3", "4", "5", "6", "7"]
        # lbl = ["5", "7", "11", "14", "17"]
        lbl = ["Age 5", "Age 7", "Age 11", "Age 14", "Age 17"][(swpstn + 1 - 3):]
        ...
        logging.info(desc)
        # Plotting means with error bars
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'sendif_{rskfctr}_{rtp}.png')
        # lbl = ["4", "5", "6"]
        # lbl = ["7", "11", "14"]
        lbl = ["Age 7", "Age 11", "Age 14"]
        ...
        logging.info(desc)
        # Plotting means with error bars
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'truancdif_{rskfctr}_{rtp}.png')
        # lbl = ["5", "6"]
        # lbl = ["11", "14"]
        lbl = ["Age 11", "Age 14"]
        ...
        logging.info(desc)
        # Plotting means with error bars
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'exclusdif_{rskfctr}_{rtp}.png') 
        # lbl = ["5", "6"]
        # lbl = ["11", "14"]
        lbl = ["Age 11", "Age 14"]
        ...
        plt.grid(True, 'major', 'y', ls='--', lw=.5, c='k', alpha=.3)
        plt.legend(["Hospitalisation", "Disability", "Conduct disorder", "SEN", "Truancy", "Exclusion"], loc='upper left')
        plt_path = os.path.join(lfsm, f'output/{rskfctr}/figures', f'cstsrcdif_{rskfctr}_{rtp}.png')  # Define your output file path here
        # os.remove(plt_path)
        plt.savefig(plt_path, bbox_inches='tight')
        plt.show()
    ...
    for rtp in ["CT", "CTI", "T", "RT", "RTI"]: 
        ...
        if rtp in ("CT", "CTI"):
            for i in range(1, nint + 1):
                daoc[f'int{i}'] = daoc[f'int{i}'] * cohsz
        ...
        for i in range(1, nint+1):
            ...
            dtwc[f'int{i}'] = cstsm(d_i, 0.035, swpstn, cohsz, ctp=rtp)
            dtuc[f'int{i}'] = cstsm(d_i, 0, swpstn, cohsz, ctp=rtp)
        if rtp in ("CT", "CTI"):    
        ...
        del wb['var']
        logging.info('Intervention effects on wellbeing')
        dtww = lfstfm(wb, 0.035, swpstn, cohsz, wbval.loc['wbnv', 'ttl'], ctp=rtp)
        
        logging.info('Intervention effects on wellbeing (undiscounted)')
        dtuw = lfstfm(wb, 0, swpstn, cohsz, wbval.loc['wbnv', 'ttl'], ctp=rtp)
        
        logging.info('Intervention effects on wellbeing (LB value)')
        dtwwl = lfstfm(wb, 0.035, swpstn, cohsz, wbval.loc['wblv', 'ttl'], ctp=rtp)
        logging.info('Intervention effects on wellbeing (UB value)')
        dtwwu = lfstfm(wb, 0.035, swpstn, cohsz, wbval.loc['wbuv', 'ttl'], ctp=rtp)  
        logging.info('Intervention effects on wellbeing (SS value)')
        dtwws = lfstfm(wb, 0.035, swpstn, cohsz, wbval.loc['wbsv', 'ttl'], ctp=rtp)          
        ...
        if rtp in ("CT", "CTI"):
            ...
            dtwwl.loc['wbvl'] = dtwwl.loc['wbvl'] / 1000000 
            dtwwu.loc['wbvl'] = dtwwu.loc['wbvl'] / 1000000 
            dtwws.loc['wbvl'] = dtwws.loc['wbvl'] / 1000000 
            
        #Correctly label rows and columns
        dtww = dtww.rename(index=var_dict) 
        dtuw = dtuw.rename(index=var_dict) 
        dtwwl = dtww.rename(index=var_dict) 
        dtwwu = dtww.rename(index=var_dict) 
        dtwws = dtww.rename(index=var_dict) 
        for i in range(1, nint+1):
            dtww = dtww.rename(columns={f"int{i}": f"Scenario {i}"})
            dtuw = dtuw.rename(columns={f"int{i}": f"Scenario {i}"})
            dtwwl = dtww.rename(columns={f"int{i}": f"Scenario {i}"})
            dtwwu = dtww.rename(columns={f"int{i}": f"Scenario {i}"})
            dtwws = dtww.rename(columns={f"int{i}": f"Scenario {i}"})
        ...
        if rskfctr in ("incqnt0", "incqntim0", "incqntnc0", "linctb0"):   
            ...
            # if rtp == "CT":
            #     for i in range(1, nint + 1):
            #         dapc[f'int{i}'] = dapc[f'int{i}'] * cohsz
            #         dapu[f'int{i}'] = dapu[f'int{i}'] * cohsz    
            ...
            if rtp in ("CT", "CTI"):       
                #Convert values to millions    
                dapc = dapc * cohsz / 1000000 
                dapu = dapu * cohsz / 1000000   
        ...
        with open(os.path.join(lfsm, f'output/{rskfctr}/tables', f'wellbyvalcomp_{rskfctr}_{rtp}.tex'), 'w', encoding='utf-8') as f:
                ...
                for i in range(1, nint + 1):
                    f.write(r' & ' + str(round(dtwwl.loc["Value of WELLBYs", f"Scenario {i}"])))
                f.write(r'\\' + '\n')
                if rtp in ("T", "RT", "RTI"):
                ...
                for i in range(1, nint + 1):
                    f.write(r' & ' + str(round(dtwwu.loc["Value of WELLBYs", f"Scenario {i}"])))
                f.write(r'\\' + '\n')
                if rtp in ("T", "RT", "RTI"):
                ...
                for i in range(1, nint + 1):
                    f.write(r' & ' + str(round(dtwws.loc["Value of WELLBYs", f"Scenario {i}"])))
