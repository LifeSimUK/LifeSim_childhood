...
class Person1:
    ...
    def __init__(self, x, betas, res, probs, co, bo, sw):
        ...
        for c in range(sn, ln):
             continuous_outcomes['sdqconduct' + str(c)] = np.clip(continuous_outcomes['sdqconduct' + str(c)], a_max=10, a_min=0)
             continuous_outcomes['sdqemotion' + str(c)] = np.clip(continuous_outcomes['sdqemotion' + str(c)], a_max=10, a_min=0)
             continuous_outcomes['sdqpeer' + str(c)] = np.clip(continuous_outcomes['sdqpeer' + str(c)], a_max=10, a_min=0)
             continuous_outcomes['sdqhyper' + str(c)] = np.clip(continuous_outcomes['sdqhyper' + str(c)], a_max=10, a_min=0)
             continuous_outcomes['sdqprosoc' + str(c)] = np.clip(continuous_outcomes['sdqprosoc' + str(c)], a_max=10, a_min=0)
             continuous_outcomes['sdqinternal' + str(c)] = np.clip(continuous_outcomes['sdqinternal' + str(c)], a_max=20, a_min=0)
             continuous_outcomes['sdqexternal' + str(c)] = np.clip(continuous_outcomes['sdqexternal' + str(c)], a_max=20, a_min=0)
