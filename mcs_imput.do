...
bdgcseme7 /// end of school results
...
agemb smokepreg1 drinkpreg1 malm1   /// mother's perinatal characteristics
...
buag1ch1 buag2ch1 buag3ch1 buag4ch1 buag1ch2 buag2ch2 buag3ch2 buag4ch2 buag1ch3 buag2ch3 buag3ch3 buag4ch3 buag1ch4 buag2ch4 buag3ch4 buag4ch4 buag1ch5 buag2ch5 buag3ch5 buag4ch5 buag1ch6 buag2ch6 buag3ch6 buag4ch6 buag1ch7 buag2ch7 buag3ch7 buag4ch7  /// Number of children in hosehold by age group
...
(logit, augment) bdgcseme7 /// end of school results																														///  removed: 
...
(logit, augment) smokepreg1 drinkpreg1 ///																																	///  removed: 
(regress) malm1   /// initial family variables 																												### Poisson		///  removed: 
...
(truncreg) buag1ch3 buag2ch3 buag3ch3 buag4ch3 /// Number of children in hosehold by age group																				///  removed:  buag1ch1 buag2ch1 buag3ch1 buag4ch1 buag1ch2 buag2ch2 buag3ch2 buag4ch2 buag1ch4 buag2ch4 buag3ch4 buag4ch4 buag1ch5 buag2ch5 buag3ch5 buag4ch5 buag1ch6 buag2ch6 buag3ch6 buag4ch6 buag1ch7 buag2ch7 buag3ch7 buag4ch7
///(truncreg) numppl1 numppl2 numppl3 /// Number of people in hosehold																						### Poisson		///  removed:  numppl4 numppl5 numppl6 numppl7
///(truncreg) numchld1 numchld2 numchld3  /// Number of children in household																				### Poisson		///  removed:  numchld4 numchld5 numchld6 numchld7
...
///(regress) cma1 /// Parental attachment																													### Poisson		///  removed: 
...
(regress) bthwt gestweeks1 /// child initial characteristics  ///removed: ITtotal
...
add(50) force rseed (4000) dots noisily augment
...
*Delayed development
// mi passive: gen deldev3 = zcog3 <= -1.282 // Bottom 10%
// mi passive: gen deldev3 = zcog3 <= -1 // Bottom 15% (https://bmjopen.bmj.com/content/bmjopen/9/6/e024851.full.pdf)
// mi passive: gen deldev3 = zcog3 <= -0.674 // Bottom 25% (https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0234414)
mi passive: gen deldev3 = zcog3 <= -0.553 // Bottom 30%
mi passive: replace deldev3 = . if zcog3 == .
*/
egen zbasnv3 = std(basnv3)
egen zbaspc3 = std(baspc3)
egen zbasps3 = std(basps3)
gen cog3 = zbasnv3 + zbaspc3 + zbasps3
egen zcog3 = std(cog3)
*Delayed development
// mi passive: gen deldev3 = zcog3 <= -1.282 // Bottom 10%
// mi passive: gen deldev3 = zcog3 <= -1 // Bottom 15% (https://bmjopen.bmj.com/content/bmjopen/9/6/e024851.full.pdf)
// mi passive: gen deldev3 = zcog3 <= -0.674 // Bottom 25% (https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0234414)
gen deldev3 = zcog3 <= -0.553 // Bottom 30%
replace deldev3 = . if zcog3 == .
**Sweep 4
/*Age adjust scores
...
mi passive: gen malm1_r = round(malm1)
mi passive: replace malm1_r = 0 if malm1_r < 0
mi passive: replace malm1_r = 24 if malm1_r > 24 & malm1_r != .
drop malm1
mi rename malm1_r malm1

drop malm_clin
mi passive: gen malm_clin = malm1 >= 4 & malm1 != .
mi update

***Number of people in household
forval i = 1/3{
mi passive: gen numppl`i' = round(buag1ch`i' + buag2ch`i' + buag3ch`i' + buag4ch`i')
...
forval i = 1/3{
mi passive: gen numchld`i' = round(buag1ch`i' + buag2ch`i' + buag3ch`i' + buag4ch`i')
...
forval i = 1/3{
mi passive: gen numadlt`i' = round(buag4ch`i')
...
*** Income quintile interaction terms

forval i = 1/5{
		gen lincqnt123_`i' = laincome123 if incqnt123 == `i'
		replace lincqnt123_`i' = 0 if missing(lincqnt123_`i')
}
...
*/
mi passive: gen pretrm_3 = gestweeks1 < 28 & gestweeks1 != .
mi passive: gen pretrm_2 = gestweeks1 >= 28 & gestweeks1 < 32 & gestweeks1 != .
mi passive: gen pretrm_1 = gestweeks1 >= 32 & gestweeks1 < 37 & gestweeks1 != .

mi update
*** Low birthweight
*Weeks of gestation
mi passive: gen gestwks = round(gestweeks1)
...
mi passive: gen lwht2sd3 = height3 <= sd2 if height3 != . & sd2 != .
mi passive: gen lwht15sd3 = height3 <= sd15 if height3 != . & sd15 != .
mi update

drop p10 months sd2 sd15 p90
